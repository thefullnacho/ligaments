...
**Built (n=1):** when I resolved the divergence above, I pinned the decision with a regression
test in the repo that had to honour it, so the old behaviour cannot silently come back. One
canonical statement, one test, one repo. That is the mechanism working at the smallest possible
scale.
